...
**45,000 was set from the worst ratio then observed, not the average.** 23.8 against a
...
stayed at 45,000 and the decision was **deferred, not resolved**. What would settle it was several
`measure_only: true` replays of a diff in the 40,000–46,000 band: one run is not a sample, and this
row is why. The paragraph that follows is those replays; everything above it is the record of how
the number got to 45,000 and reads in the past tense for that reason.
...
| #720 (again) | 43,987 | 4 | **265,097** | 65,300 | **6.03** | clean verdict, 10m19s |
...
that spends the ceiling on nothing — never came close in any sample: the heaviest run emitted
297,450 characters, a factor of 4.98 under the budget, and `DiffCapTests` pins that bound at four.
...
cap since 90,000 has used, is 88,493, and **90,000 is 73% of the fill point at twice the worst
observed ratio.** At that doubled ratio it spends about 281,200 tokens reasoning, 73% of the
ceiling, which is the same "a quarter left over" criterion `DiffCapTests` has always checked; at
the worst ratio actually observed it spends about 140,600, and at the median it is never close.
...
**Two things about that margin, said rather than rounded away.** 90,000 is 1.7% *above* the 88,493
the derivation gives: rounded up past its own margin, inside the noise of a seventeen-sample ratio,
and the floor test allows it deliberately. And the headroom the second check measures is 2.4% —
281,200 tokens against the 288,000 it allows — so a future sample reasoning above about 6.18
characters per character reddens the suite with the cap untouched. That is the intended alarm and
not a surprise: the worst observed is 6.03 and the run-to-run swing is 1.6×, which puts it in
reach. Neither number is derivable from the prose alone, which is why `DiffCapTests` computes both
from the samples rather than restating them.
...
seven issues already needed splitting at 90,000; at 45,000 most changes became two or three pull
...
measured failure at 60,863, and the cap sat just under the success.
