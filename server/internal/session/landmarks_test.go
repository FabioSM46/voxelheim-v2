...
func TestWelcomeSendsLandmarksBeforeStreamingAndRefusesForgedLists(t *testing.T) {
	m := startMarking(t, t.TempDir(), testAccount(73), "Eivor", 1)
	waitUntil(t, "initial landmark list", func() bool { return slices.Contains(m.sink.kindsReceived(), vnet.PayloadLandmarkList) })
	kinds := m.sink.kindsReceived()
	at := slices.Index(kinds, vnet.PayloadLandmarkList)
	if chunk := slices.Index(kinds, vnet.PayloadChunkData); chunk >= 0 && chunk < at {
		t.Fatal("chunk overtook initial landmarks")
	}
	...
	waitUntil(t, "masked remote tile", func() bool { return len(m.sink.mapTilesReceived()) == 1 })
	...
	frame, err := protocol.EncodeLandmarkList(protocol.LandmarkList{Landmarks: []protocol.Landmark{{LandmarkID: 9, X: 7091, Z: -93680, Kind: vnet.LandmarkKindPortal}}})
	if err != nil {
		t.Fatal(err)
	}
	m.conn.in <- frame
	...
	if count != 1 {
		t.Fatalf("forge elicited %d lists; want only the welcome's one", count)
	}
}
