...
func TestInstanceMarkerRequestsCannotReadOrMutateTheOpenWorldLedger(t *testing.T) {
	...
	conn.in <- protocol.EncodeChatRequest(protocol.ChatRequest{Text: "markers checked"})
	waitUntil(t, "marker requests consumed", func() bool { return len(frames.chatMessages()) == 1 })
	...
	conn.in <- protocol.EncodeChatRequest(protocol.ChatRequest{Text: "returning"})
	waitUntil(t, "return movement accepted", func() bool { return len(frames.chatMessages()) == 2 })
	for range 10 {
		cfg.Instances.Step()
	}
	conn.in <- protocol.EncodePortalRequest(protocol.PortalRequest{HasArch: true, Arch: change.ExitArch})
