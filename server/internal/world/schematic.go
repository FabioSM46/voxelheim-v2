...
// The keep is not here. It is 63×63×68 and 4,284 rows of layer literal on its own, so it
// lives in schematic_keep.go and the four storey files that one concatenates —
// schematic_keep_ground.go, _middle.go, _roofs.go and _spires.go. The first split is
// because a drawing an order of magnitude larger than the other three would bury them;
// the second is because GitHub will not return a patch for a file that size, and the
// reason is written out where somebody about to undo it will read it.
