...
// **The set-aside, and what proves it did not migrate or delete.** A v6 players
// directory has no worn-equipment slots, so it is moved whole and a fresh v7 one is opened.
// Every byte that was in it is still there, under a name nothing will write to; the
// world starts with no characters at all rather than inventing equipment for old lives.
func TestAV6DirectoryIsSetAsideAndNotMigrated(t *testing.T) {
	t.Parallel()
	...
	// need to parse as v6 lives: startup reads only this store's magic and version before
...
func TestAV7DirectoryMigratesLosslessRecordsAndRefusesSilverStacks(t *testing.T) {
	...
	if len(migrated) != len(old)+slotSize+4+1 {
		t.Errorf("the migrated record is %d bytes, want v7's %d plus one slot, the purse and the learned set", len(migrated), len(old))
	}
	...
	aside := store.SetAside()
	if base := filepath.Base(aside); !strings.HasPrefix(base, playersDirName+supersededSuffix+"10.") {
		t.Errorf("the v7 directory was kept as %q, want players.pre-v10.<timestamp>", base)
	}
...
func TestASecondSetAsideDoesNotOverwriteTheFirst(t *testing.T) {
	...
	for round := byte(1); round <= 2; round++ {
		...
		if err := os.WriteFile(filepath.Join(players, name), v6Record(round), 0o600); err != nil {
			t.Fatalf("writing %s: %v", name, err)
		}
		store, err := OpenStore(worldDir)
		if err != nil {
...
// v6Record is what the immediately previous build wrote as far as startup needs to
// know: this store's magic, format version 6, and a body v7 deliberately never parses.
func v6Record(seed byte) []byte {
