...
func (h *structureHarness) joinAs(entityID uint64, playerID identity.PlayerID, pos [3]float32) (*Player, *dropSink) {
	...
	out := &dropSink{}
	player, err := h.sim.Join(entityID, playerID, testCharacterName, pos, testAppearance(), nil, out.deliver)
	if err != nil {
...
func TestOneIdentityCannotBeTwoPlayersAtOnce(t *testing.T) {
	...
	out := &dropSink{}
	if _, err := h.sim.Join(2, testPlayerID(1), testCharacterName, [3]float32{0.5, 64, 0.5}, testAppearance(), nil, out.deliver); err == nil {
