...
//
// **The soak owns the server because a cost is only attributable to a process you can
// name.** Starting it here gives the run a pid to read /proc for, a certificate fingerprint
// straight out of the startup line instead of copied by hand, a port the kernel chose so two
// runs never collide, and one command in the ADR instead of a page of shell.
//
// Nothing about its configuration is invented here: every flag below is one this command was
// given, and the resulting command line is printed in the report.

// The startup lines this command waits for.
//
// **Both are Info, so a run must not be allowed to silence them**: a server logging at warn
// would start, listen, and never say where. runOptions.validate refuses such a level and
// [serverStartDeadline] turns any other silence into a diagnostic rather than a hang —
// #935's review is where that was found.
const (
	listeningMessage   = "voxelheimd listening"
	certificateMessage = "listening with an encrypted session"
)

// serverStartDeadline bounds the wait for those two lines.
//
// A backstop rather than a tuning knob: a server given a legal log level says both within a
// second. It catches the one that starts and says something else entirely — a different
// build, or a flag it does not know — where the alternative is waiting for a signal nobody
// will send.
...
func startServer(ctx context.Context, o options, run runOptions) (*serverProcess, error) {
	...
	starting := time.NewTimer(serverStartDeadline)
	defer starting.Stop()
	select {
	case err := <-ready:
		if err != nil {
	...
	case <-starting.C:
		_ = cmd.Process.Kill()
		return nil, fmt.Errorf("the server did not say where it was listening within %v; its last output was:\n%s",
			serverStartDeadline, server.tail.text())
	case <-ctx.Done():
		_ = cmd.Process.Kill()
		return nil, ctx.Err()
	}
...
//
// Two jobs in one goroutine because they are one stream: the two startup lines the run needs
// before it can dial, and the running count of the refusals the relay makes silently. A line
// that is not JSON is kept for the tail rather than treated as an error — a panic is exactly
// what a failing run needs to be able to print.
...
// it: this covers the process's whole life, joins included, while the sampler covers the
// window alone. A disagreement larger than the join phase explains means the sampler is
// wrong.
...
//
// **An assumption, stated rather than hidden.** `USER_HZ` is a compile-time constant of the
// kernel's userspace ABI, it is 100 on every Linux this project runs on, and the portable
// way to ask is `sysconf(_SC_CLK_TCK)` — cgo, which this module has none of.
// [serverProcess.stop]'s rusage cross-check is what catches the day it stops being true.
const clockTicksPerSecond = 100

// readUsage reads one process's processor time and resident size out of /proc.
//
// Linux only, and it says so by failing rather than by a build tag: a tag would drop this
// file out of vet and the cross-compile gates for the sake of a platform nobody runs it on.
