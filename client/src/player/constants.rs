...
//! it fits. They are also the *grid* the body is cut on — [`super::appearance`] divides
//! them into twelfths and thirty-sixths rather than holding a size of its own, so a change
//! here moves every part of the rig with it.
//!
...
//
// The body's own proportions are not among them any more. They were, while the body was one
// capsule inscribed in the collision box; a rig of a dozen-odd boxes has relationships a `const`
// expression cannot state, so `super::appearance` asserts them as tests instead — which is
// also where the two parts that deliberately leave the box are named.
