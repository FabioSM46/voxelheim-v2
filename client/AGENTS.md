...
| `player/saddle.rs` | the camera-space saddle view shown by the newest authoritative local mount projection: the world horse's own head, pointed ears, eyes, neck and mane at one scale, a view-only continuation below the neck, and translation adapted between narrow and default field of view, and the reins from the shared first-person hands to the bit | draw a world horse, re-type a size the horse has, predict a mount transition, or decide any mounted action legality |
...
| `ui/overhead.rs` | the small over-head health bars — red over what fights, yellow over what can be hit and does not fight back, green over the other players — placed from the interpolated snapshot through the name plates' reach, line of sight and dwell, and filtered by the Interface tab's setting | read health as death, send anything, keep a bar for an entity the newest snapshot omits, or let a colour decide who may be hit |
