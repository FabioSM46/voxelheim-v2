...
## 10. Audio e Voce di Prossimità
*   **La voce viene prima:** La chat vocale di prossimità è la prima funzione audio del gioco. Musica ed effetti sonori arrivano più tardi, sullo stesso mixer del client e sugli stessi bus, così che un giocatore regoli la voce senza abbassare il resto e viceversa.
*   **Prossimità, non canali:** Si sente chi è vicino nel mondo. Non esistono canali globali né stanze permanenti: la distanza è la regola, e la voce si attenua fino a sparire oltre la portata udibile.
*   **Il server decide chi sente chi:** L'audibilità è una decisione autoritativa come qualunque altra. Il client cattura, codifica e riproduce; non stabilisce mai da solo di poter sentire qualcuno né di poter essere sentito. Un client modificato può tacere, ma non può ascoltare oltre ciò che il server gli invia.
*   **Un frame vocale è un dato personale:** Il server lo inoltra e non lo registra mai. Nessuna scrittura su disco, nessuna riga di log con dentro l'audio, nessuna conservazione: la voce attraversa il server e non vi resta.
*   **Push-to-talk di default, attivazione vocale a scelta:** Il gioco parte con il microfono chiuso e si apre solo mentre il tasto è premuto. L'attivazione automatica sulla voce esiste, ma è una scelta esplicita del giocatore, mai l'impostazione iniziale.
*   **L'interruttore dell'uditorio restringe, non allarga:** Il giocatore può scegliere di parlare a tutti quelli vicini oppure al solo party. La seconda opzione è un filtro dentro la prossimità: non permette in nessun caso di raggiungere qualcuno fuori dalla portata udibile. Un compagno di party dall'altra parte della mappa non sente nulla.
*   **Silenziare è una decisione locale:** Ogni giocatore può silenziare un altro per sé. È l'unica parte della catena che il client decide da solo, perché non toglie nulla a nessun altro.
